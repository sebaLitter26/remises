...
import { FnModificacionModif } from './funciones_modificacion_modif.component';
import { BusquedaCallejeroModif } from './busqueda_callejero_modif.component';
...
@Component({
    ...
    providers: [BusquedaCallejeroModif],
})

export class ModificacionComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    //private busquedaCallejeroComponent: BusquedaCallejero;
    //FLAGS
    ...
    constructor(
        ...
        private busquedaCallejeroComponent: BusquedaCallejeroModif,
    ) {
        this.funciones = new FnModificacionModif(this._ubi);
        console.log('modificacionComponent');
        this.buscadorCallejero = new BusquedaCallejeroModif(this._ubi, this.store);
    }
    ...
    ngOnInit() {
        ...
        */
        ...
        this.formGroup.get("observacion").valueChanges.pipe(
              startWith<string | any>(''),
              debounceTime(500),
              distinctUntilChanged()
          ).subscribe(respuesta=>{
            if(this._ubi.camposObligatorios){
              //console.log("Hay campos obligatorios");
              if(respuesta){
                //console.log("Tengo campos obligatorios y los cargué")
                this.validacionOptativos=true
              }else{
                //console.log("Tengo campos obligatorios pero no los cargué")
                this.validacionOptativos=false
              }
            }else{
                //console.log("No tengo campos obligatorios")
                this.validacionOptativos=true
            }
          });
    }
  ...
  */
    ...
    reiniciar_datos() {
        ...
        this.nombreCalle = "";
        this.cuandoLlega = "Llega ahora";
        this.observacion = '';
        this.entreCalles = '';
        this.datosAdministrativos= null;
    ...
    limpiar_formulario() {
        ...
        if(this._ubi.camposAdministrativos){
          this._ubi.camposAdministrativos.map(campo=>{
            if(campo.valor)
              delete campo.valor;
          });
        }
    ...
    programarViaje() {
        //console.warn(this.formGroup.get("viajeProgramado").value);

        if (this.formGroup.get("viajeProgramado").value) {
            this.cuandoLlega = "Llega ahora";
            setTimeout(() => this.formGroup.patchValue({
                viajeProgramado: false
            }), 400);
        }
        else {
            this._dialog.abrir({
                ...
                height: '450px',
                data: {
                    titulo: "Programar viaje",
                    horaMinima: this.horaMinima,
            ...
            }, 'modificacion').subscribe(datos => {
                if (datos) {
                    if (datos.fechaeventual) {
                        console.log("cerro dialog", datos);
                        this.programado = datos;
                        console.log(this.programado);
                        this.formGroup.patchValue({
                            viajeProgramado: true
                        })
                        var localLocale = moment(this.programado['fechaeventual']);
                        moment.locale('es');
                        localLocale.locale(false);
                        if (this.programado['coincidencia'])
                            this.cuandoLlega = 'Llega hoy a las ' + this.programado['horaeventual'];
                        else
                            this.cuandoLlega = 'Llega el' + ' ' + localLocale.format('dddd D') + "\n a las " + this.programado['horaeventual'];
                    }
                } else {
                    this.cuandoLlega = "Llega ahora";
                    this.formGroup.patchValue({
    ...
    cargarObservaciones() {
        this._dialog.abrir({
            width: '500px',
            height: (this._ubi.camposAdministrativos.length>0) ? '550px' : '300px' ,
            data: {
              titulo: "Observaciones",
              datosAdministrativos: (this._ubi.camposAdministrativos.length>0) ? true : false ,
            ...
            if (datos) {
                ...
                this.datosAdministrativos={};
                  this._ubi.camposAdministrativos.map(campo=>{
                    //Object.assign(this._ubi.camposAdministrativos, {'valor': datos[campo.key]});
                    this.adicionalesAdministrativos=true;
                    Object.assign(this.datosAdministrativos, {[campo.key]:datos[campo.key]});
                    //console.log(campo);
                    Object.assign(campo, {valor:datos[campo.key]});
                  });
                  console.warn(this._ubi.camposAdministrativos);
    ...
    onSubmit() {
        ...
        if (this.cuandoLlega == 'Llega ahora') {
            var alto = '250px';
        } else {
            formulario.fechaeventual = moment(this.programado['fechaeventual'], 'MM/DD/YYYY').format('DD/MM/YYYY');
            formulario.horaeventual = this.programado['horaeventual'];
            if (!this.programado['tiempoAntes'])
                formulario.tiempoantes = 10;
            else
                formulario.tiempoantes = this.programado['tiempoAntes'];
            var alto = '350px';
        }
        ...
        if (this.observacion != '' && this.entreCalles != '') {
            formulario['observacion'] = this.entreCalles + ' - ' + this.observacion;
        }
        else {
            if (this.observacion != '')
                formulario['observacion'] = this.observacion;
            else if (this.entreCalles != '')
                formulario['observacion'] = this.entreCalles;
        }

        /*Si se ingresan los datos de la "intersección" se envía calle como "'Calle' y 'Intersección'"  */
        ...
        console.warn("Abro el POPUP de Confirmar Viaje");
        this._dialog.abrir({
            width: '400px',
            height: alto,
